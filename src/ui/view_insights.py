"""
ui/view_insights.py
Visão de Insights — cards FILO clicáveis com busca fuzzy e edição inline.
"""
...
class InsightsView(ctk.CTkFrame):
    ...
    def focus_search(self):
        """Foca o campo de busca — chamado via Ctrl+F do dashboard."""
    ...
    def _apply_filter(self):
        ...
        if q:
            self._filtered = [
                e for e in self._insights
                if fuzzy_match(q, f"{e.content} {e.title} {' '.join(e.tags)}")
            ]
        else:
            self._filtered = list(self._insights)
        self._render()
    ...
    def _render(self):
        ...
        self.status_label.configure(
            text=f"{n} de {total} insights" if self._search_text else f"{total} insights"
        )
    ...
    def _edit_selected(self):
        ...
        if not row:
            return
        self._render_inline_editor(entry, row)

    def _render_inline_editor(self, entry: Entry, row: ctk.CTkFrame):
    ...
    def _copy_selected(self):
        ...
        self.after(2000, lambda: self.status_label.configure(
            text=f"{len(self._filtered)} insights"
        ))
