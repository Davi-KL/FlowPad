"""
ui/view_insights.py
Visão de Insights — cards FILO com conteúdo e timestamp.
"""
...
class InsightsView(tk.Frame):
    ...
    def _build_ui(self):
        ...
        for label, bg, cmd in [
            ("📋 Copiar  [C]",    COLORS["accent2"], self._copy_selected),
            ("🗂 Arquivar  [A]",  COLORS["accent"],  self._archive_selected),
    ...
    def _render(self):
        ...
        for entry in self._insights:
            ...
            self.listbox.insert("end", f"  💡  {line}{tag_hint}  —  {ts}")
        self.status_label.config(text=f"{len(self._insights)} insights")
    ...
    def _on_select(self, _event=None):
        ...
        entry = self._insights[self._selected_idx]
        self.detail_label.config(text=entry.content[:500], fg=COLORS["text"])

    def _get_selected(self) -> Entry | None:
        if 0 <= self._selected_idx < len(self._insights):
            return self._insights[self._selected_idx]
        return None
    ...
    def _copy_selected(self):
        ...
        self.after(2000, lambda: self.status_label.config(text=f"{len(self._insights)} insights"))
